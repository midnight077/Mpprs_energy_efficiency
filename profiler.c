 ...
 *
 * REQUIRED SOFTWARES / STEPS (on HiPeC server these are already done)
...
int NUM_PHYSICAL_CORES_PER_SOCKET=0, NUM_THREADS_PER_PHYSICAL_CORE=0;
int NUM_SOCKETS=0, NUM_CORES=0, NUM_LOGICAL_CORES=0;
int* socket_core = NULL;
...
void gather_machine_info() {
    ...
    pclose(lscpu);
    assert(NUM_PHYSICAL_CORES_PER_SOCKET > 0 && NUM_THREADS_PER_PHYSICAL_CORE > 0 && NUM_SOCKETS > 0);
